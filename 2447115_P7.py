...
def main():
    ...
    if uploaded_file is not None or use_default:
        ...
        potential_targets = [col for col in df.columns if df[col].dtype == 'object' or df[col].nunique() <= 10]
        ...
        # Process data first to show available features
        ...
        if st.sidebar.button("Run Analysis", type="primary"):
            run_analysis(df, missing_strategy, create_features, train_ratio, target_column, selected_features)
        ...
        - **Interactive Visualizations**: Professional-grade charts and metrics
...
def get_available_features(df, target_column):
    ...
    # Encode categorical variables to get full feature list
    df_encoded = df.copy()
    feature_columns = []
...
def run_analysis(df, missing_strategy, create_features, train_ratio, target_column, selected_features):
    """Execute comprehensive ML pipeline"""
    
    with st.spinner("Processing data and training models..."):
        # Data preprocessing
        df_processed = preprocess_data(df, missing_strategy, create_features, show_output=True)
        ...
        st.markdown("### Selected Features for Modeling")
        ...
        # Display results
        display_results(results, target_mapping, feature_names)

def display_results(results, target_mapping, feature_names):
    ...
    if 'feature_importance' in results['Random Forest']:
        top_3_impact = importance_df.tail(3)['Importance'].sum()
    else:
        top_3_impact = 0
    ...
    2. **Feature Selection**: Using {len(feature_names)} selected features for prediction
    3. **Feature Impact**: Top 3 predictors contribute {top_3_impact:.1%} of model decisions
    4. **Risk Classification**: Model successfully identifies patterns in student outcomes
